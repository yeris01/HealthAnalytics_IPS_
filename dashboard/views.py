...
def calcular_kpis():
    ...
    return {
        ...
        'edad_promedio': round(
            Paciente.objects.aggregate(Avg('edad'))['edad__avg'] or 0, 1
        ),
        'imc_promedio': round(
            Paciente.objects.aggregate(Avg('imc'))['imc__avg'] or 0, 1
        ),
        'glucosa_promedio': round(
            Paciente.objects.aggregate(Avg('glucosa'))['glucosa__avg'] or 0, 1
        ),
        'presion_promedio': round(
            Paciente.objects.aggregate(Avg('presion_sistolica'))['presion_sistolica__avg'] or 0, 1
        ),
...
def index(request):
    return render(
        ...
        {
            'kpis': calcular_kpis(),
            'logs_recientes': LogETL.objects.all()[:5]
        }
    )
...
def pacientes_api(request):
    ...
    return Response(
        PacienteSerializer(qs[:500], many=True).data
    )
...
def usuarios_api(request):
    return Response(
        UsuarioSerializer(
            Usuario.objects.all().order_by('username'),
            many=True
        ).data
    )