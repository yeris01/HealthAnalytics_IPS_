...
def transformar_dataset(df_raw):
    ...
    df.loc[df['peso'] > 300, 'peso'] = np.nan
    df.loc[df['peso'] < 10, 'peso'] = np.nan
    df.loc[df['temperatura'] < 33, 'temperatura'] = np.nan
    df.loc[df['temperatura'] > 43, 'temperatura'] = np.nan
    df.loc[df['presión_sistólica'] > 250, 'presión_sistólica'] = np.nan
    df.loc[df['presión_sistólica'] < 50, 'presión_sistólica'] = np.nan
    df.loc[df['glucosa'] < 20, 'glucosa'] = np.nan
    df.loc[df['saturación_oxígeno'] > 100, 'saturación_oxígeno'] = np.nan
    df.loc[df['saturación_oxígeno'] < 50, 'saturación_oxígeno'] = np.nan
